...
class main():
    ...
    def __init__(self, pyjama):
        ...
        self.pyjama.preferences.register_plugin("LibreFM", self.create_preferences, self.save_preferences)

        # login to libre.fm
        self.scrobbler = None

        if self.get_session():
            logging.debug('libre.fm plugin loaded')
        else:
            logging.debug('libre.fm plugin loaded but scrobbling isn\'t available')
...
#        thr.start()
    ...
    def ev_nowplaying(self, track):
        ...
        sleep(20)
        """ for sending now playing notification to libre.fm """
        logging.debug('nowplaying: %s' % track)

        if self.scrobbler is None:
            self.get_session()

        if self.pyjama.settings.get_value('LIBREFM','SCROBBLING') and self.scrobbler is not None:
            try:
                ...
                logging.error('can\'t send nowplaying info to libre.fm')
                self.get_session()
            except Exception, e:
                logging.error(e)
            else:
                logging.info('nowplaying %s - %s send to libre.fm' % (track.artist_name, track.name))
                self.pyjama.setInfo("librefm - nowplaying send")
...
#        thr.start()
    ...
    def ev_scrobble(self,track):
        ...
        if self.pyjama.settings.get_value('LIBREFM','SCROBBLING') and self.scrobbler is not None:
            ...
            try:
                ...
                logging.warn('libre.fm plugin: can\'t scrobble song')
                self.get_session()
            except Exception, e:
                logging.error(e)
                #raise
            else:
                logging.info('song %s - %s send to libre.fm' % (track.artist_name, track.name))
                self.pyjama.setInfo("librefm - scrobbled")
    ...
    def ev_player_status(self, status, cursec = None, duration = None):
        if status == "playing":
            ...
            if (percentage > 0.5 or (cursec > 240 and cursec <= duration)) and duration > 30 and self.pyjama.player.cur_playing.id != self.last_scrobbled:
                #print "send this to libre.fm ;) <<<< ", self.cursec, '/',  self.duration
                self.ev_scrobble(self.pyjama.player.cur_playing)
                self.last_scrobbled = self.pyjama.player.cur_playing.id

    def create_preferences(self):
        login_value=self.pyjama.settings.get_value('LIBREFM','LOGIN','login', str)
        pass_value=self.pyjama.settings.get_value('LIBREFM','PASS', 'password', str)
        scrobble_value=self.pyjama.settings.get_value('LIBREFM','SCROBBLING', False)
        ...
        self.check = gtk.CheckButton(_("Scrobble to libre.fm?"))
        ...
        if login_value != "login":
            lbl = MyLinkButton("http://alpha.libre.fm/user/%s"  % login_value, "Open my page @librefm")
            lbl.show()
    ...
    def save_preferences(self):
        self.pyjama.settings.set_value('LIBREFM','LOGIN', self.login.get_text())
        logging.debug('new login: %s' % self.login.get_text())

        self.pyjama.settings.set_value('LIBREFM','PASS', self.password.get_text())
        logging.debug('new pass: %s' % self.password.get_text())

        self.pyjama.settings.set_value('LIBREFM','SCROBBLING', self.check.get_active())
