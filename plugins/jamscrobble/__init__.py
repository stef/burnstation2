...
class main():
    ...
    def __init__(self, pyjama):
        ...
        self.pyjama.preferences.register_plugin("JamendoScrobble", self.create_preferences, self.save_preferences)

        # login to jamendoscrobble
        self.scrobbler = None

        if self.get_session():
            logging.debug('jamendoscrobble plugin loaded')
        else:
            logging.debug('jamendoscrobble plugin loaded but scrobbling isn\'t available')
...
#        thr.start()
    ...
    def get_session(self):
        if self.pyjama.settings.get_value('JamendoScrobble','SCROBBLING'):

            login=str(self.pyjama.settings.get_value('JamendoScrobble','LOGIN'))
            password=str(self.pyjama.settings.get_value('JamendoScrobble','PASS'))

            try:
                # pyjama has own jamendoscrobble clien id 'pyj'
...
#        thr.start()
    ...
    def ev_nowplaying(self, track):
        """ for sending now playing notification to jamendoscrobble """
        logging.debug('nowplaying: %s' % track)

        if self.scrobbler is None:
            self.get_session()

        if self.pyjama.settings.get_value('JamendoScrobble','SCROBBLING') and self.scrobbler is not None:
            try:
                ...
                logging.error('can\'t send nowplaying info to jamendoscrobble')
                self.get_session()
            except Exception, e:
                logging.error(e)
            else:
                logging.info('nowplaying %s - %s send to jamendoscrobble' % (track.artist_name, track.name))
                self.pyjama.setInfo("JamendoScrobble - nowplaying send")
...
#        thr.start()
    ...
    def ev_scrobble(self,track):
        """ for scrobbling to jamendoscrobble """
        ...
        if self.pyjama.settings.get_value('JamendoScrobble','SCROBBLING') and self.scrobbler is not None:
            ...
            try:
                ...
                logging.warn('jamendoscrobble plugin: can\'t scrobble song')
                self.get_session()
            except Exception, e:
                logging.error(e)
                #raise
            else:
                logging.info('song %s - %s send to jamendoscrobble' % (track.artist_name, track.name))
                self.pyjama.setInfo("JamendoScrobble - scrobbled")
    ...
    def ev_player_status(self, status, cursec = None, duration = None):
        if status == "playing":
            ...
            if (percentage > 0.5 or (cursec > 240 and cursec <= duration)) and duration > 30 and self.pyjama.player.cur_playing.id != self.last_scrobbled:
                #print "send this to jamendoscrobble ;) <<<< ", self.cursec, '/',  self.duration
                self.ev_scrobble(self.pyjama.player.cur_playing)
                self.last_scrobbled = self.pyjama.player.cur_playing.id

    def create_preferences(self):
        login_value=self.pyjama.settings.get_value('JamendoScrobble','LOGIN','login', str)
        pass_value=self.pyjama.settings.get_value('JamendoScrobble','PASS', 'password', str)
        scrobble_value=self.pyjama.settings.get_value('JamendoScrobble','SCROBBLING', False)
        ...
        self.check = gtk.CheckButton(_("Scrobble to jamendoscrobble?"))
        ...
        if login_value != "login":
            lbl = MyLinkButton("http://alpha.jamendoscrobble/user/%s"  % login_value, "Open my page @JamendoScrobble")
            lbl.show()
    ...
    def save_preferences(self):
        self.pyjama.settings.set_value('JamendoScrobble','LOGIN', self.login.get_text())
        logging.debug('new login: %s' % self.login.get_text())

        self.pyjama.settings.set_value('JamendoScrobble','PASS', self.password.get_text())
        logging.debug('new pass: %s' % self.password.get_text())

        self.pyjama.settings.set_value('JamendoScrobble','SCROBBLING', self.check.get_active())
